...
import type { DockSchedule } from "@/lib/types";
...
export async function GET(
  ...
  const { data: warehouse, error: warehouseError } = await supabase
    .from("warehouses")
    .select("id, name, timezone, booking_token, opening_time, closing_time, default_slot_duration_minutes, max_advance_booking_days, address_street, address_city, address_zip")
  ...
  const wh = warehouse as WarehouseFields;
  ...
  const { data: docks, error: docksError } = await supabase
    .from("docks")
    .select("id, warehouse_id, name, max_concurrent, is_active, sort_order")
  ...
  const activeDocks = (docks ?? []) as DockFields[];
  ...
  const { data: bookings } = await supabase
    .from("bookings")
    .select("dock_id, slot_start, slot_end, status")
    .in("dock_id", dockIds)
    .neq("status", "cancelled")
    .lt("slot_start", dayEndUtc)
    .gt("slot_end", dayStartUtc);

  const existingBookings = (bookings ?? []) as BookingFields[];
...
function formatWarehouse(wh: WarehouseFields) {
