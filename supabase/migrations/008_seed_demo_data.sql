-- ============================================================
--  SlotDock — Demo-Seeddaten
--
--  Konto:    demo@slotdock.de
--  Passwort: Demo1234!
--
--  Lager:    Logistikzentrum Hamburg-Süd
--  Rampen:   4 (general × 2, Kühlware, Sperrgut)
--  Buchungen: ~54 Einträge über ±2 Wochen ab heutigem Datum
-- ============================================================
...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    '{"full_name":"Max Mustermann"}'::jsonb,
    NOW(), NOW(), '', '', '', false
  ) ON CONFLICT (id) DO NOTHING;

  -- Profil wird per Trigger angelegt — Details ergänzen
  UPDATE profiles
  ...
  -- Rampe 3 + 4 samstags geschlossen
  ...
  INSERT INTO subscriptions (
    profile_id, stripe_customer_id, plan, status, trial_ends_at
  ) VALUES (
    uid, 'cus_demo00000000000', 'professional', 'active',
    NOW() + INTERVAL '30 days'
  );

  -- ----------------------------------------------------------
  -- 5. Buchungen — Vergangenheit (−12 … −1 Tage)
  ...
  -- ----------------------------------------------------------
  -- Heute (0): Morgen → arrived, Nachmittag → confirmed
  -- ----------------------------------------------------------
  INSERT INTO bookings (warehouse_id, dock_id, slot_start, slot_end, status, carrier_company, carrier_contact_name, carrier_email, carrier_phone, license_plate, reference_number, confirmation_code) VALUES
